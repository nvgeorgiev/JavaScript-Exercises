function solve(array) {
  ...
  for (let currentOrder of array) {
    ...
    function printResult(balance, price, typeOfDrink) {
      if (balance >= price) {
        totalIncome += price;
        console.log(`You ordered ${typeOfDrink}. Price: $${price.toFixed(2)} Change: $${(balance - price).toFixed(2)}`);
      } else {
        console.log(`Not enough money for ${typeOfDrink}. Need $${(price - balance).toFixed(2)} more.`);
      }
    }
